...
#The authors did a negative binomial regression in STATA using nbreg. The R equivalent is glm.nb from the MASS package. Nolog supresses the display of the log, which I'll keep in the model, and dispersion(constant) is to show the variance, but R shows variance in other ways so I'll omit that. link = log is defaulted in STATA's nbreg but not R's glm.nb, so I've included it here. 

model_1 <- glm.nb(data = data, terrorCounts ~ rivalry + jointDem1 + logcapratio + contiguity, link = log) 

# STATA: 'nbreg terrorCounts2 rivalry jointDem1 logcapratio contiguity, nolog cluster(dyadid)  dispersion(constant)'  

model_2 <- glm.nb(data = data, terrorCounts2 ~ rivalry + jointDem1 + logcapratio + contiguity, link = log) 
...
model_3 <- glm.nb(data = data, terrorCounts ~ rivalry + jointDem1 + logcapratio + historyl1 + historyl2 + coldwar1 + conflict1 + conflict2 + contiguity + war1 + war2, link = log) 
...
# model_4 is broken on historyl2 for some reason... 

# model_4 <- glm.nb(data = data, terrorCounts2 ~ rivalry + jointDem1 + logcapratio + historyl1 + historyl2 + coldwar1 + conflict1 + conflict2 + contiguity + war1 + war2) 
...
# No clue what the R equivlent of STATA's "inflate" - apparently inflat determines whether the count is zero. 
...
model_5 <- zeroinfl(data = data, terrorCounts ~ rivalry + jointDem1 + logcapratio + historyl1 + historyl2 + coldwar1 + conflict1 + conflict2 + contiguity + war1 + war2, dist = "negbin")
...
#this is Table 2 from the paper: 
